...
/project/docker/_download_mysql-apt-config.exp # dpkg -i mysql-apt-config_0.8.32-1_all.deb に相当
rm mysql-apt-config_0.8.32-1_all.deb

# 5. MySQLサーバーとクライアントのインストール
echo "Installing MySQL server and client..."
apt-get update
/project/docker/_install_mysql-server.exp # apt-get install mysql-server に相当
